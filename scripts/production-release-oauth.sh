...
WRANGLER=(pnpm --filter ./apps/worker exec wrangler)
...
ensure_oauth() {
  ...
  auth_type="$(printf '%s' "$auth_json" | jq -r '.type // empty' | head -n 1)"
  oauth_token="$(printf '%s' "$auth_json" | jq -r '.token // empty' | head -n 1)"
...
DB_ID="$(printf '%s' "$D1_JSON" | jq -r '([.[]? | select((.name // .database_name) == "sovereign-openapi-db")][0] | (.uuid // .id // .database_id // empty)) // empty' | head -n 1)"
if [[ -z "$DB_ID" ]]; then
...
echo "Waiting for edge convergence..."
APP_READY="{}"
PUBLIC_READY="{}"
APP_SHA=""
PUBLIC_SHA=""
...
for i in {1..20}; do
  APP_READY="$(curl -fsS "https://app.defrag.app/ready?release=$TARGET_SHA" 2>/dev/null || echo '{}')"
  PUBLIC_READY="$(curl -fsS "https://sovereign.defrag.app/ready?release=$TARGET_SHA" 2>/dev/null || echo '{}')"
  ...
  if [[ "$APP_SHA" == "$TARGET_SHA" && "$PUBLIC_SHA" == "$TARGET_SHA" ]]; then
    echo "Edge converged on attempt $i"
    break
  fi
  sleep 4
done
